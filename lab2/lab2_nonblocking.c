	...
	if(myrank==0) {
		...
		if(nprocs>1) {
			...
			for(int r=1; r<nprocs; ++r) {
				for(int i=sep_row*r; i<sep_row*(r+1); ++i) {
					for(int j=0; j<col1; ++j) {
						MPI_Irecv(&answer_blocking[i][j], 1, MPI_DOUBLE, r, 5, MPI_COMM_WORLD, &req[0]);
		...
		/* write output file */
		FILE* fp3;
		fp3=fopen("nonblocksmallresult_3426.txt", "w");
		//fp3=fopen("nonblocklargeresult_3426.txt", "w");
		if(NULL==fp3) {
		...
		MPI_Irecv(&recv_col, 1, MPI_INT, 0, 2, MPI_COMM_WORLD, &req[2]);
		/* MPI_Wait
		...
		for(int i=0; i<recv_row; ++i) {
			for(int j=0; j<recv_col; ++j) {
				MPI_Irecv(&recv_arr1[i][j], 1, MPI_DOUBLE, 0, 3, MPI_COMM_WORLD, &req[3]);
				MPI_Irecv(&recv_arr2[i][j], 1, MPI_DOUBLE, 0, 4, MPI_COMM_WORLD, &req[4]);
